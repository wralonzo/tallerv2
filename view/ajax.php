...
if (isset($_GET['q'])) {
    $datos = array();
    $nombre = $_GET['q'];
    $cliente = mysqli_query($conexion, "SELECT p.idVehiculo, pr.Nombre, p.Placa, p.Vin, p.Marca, p.tipoVehiculo, p.Color FROM
      vehiculo p
      INNER JOIN
      cliente pr
      ON
    ...
    while ($row = mysqli_fetch_assoc($cliente)) {
        $data['id'] = $row['idVehiculo'];
        $data['label'] = $row['Nombre'];
        $data['direccion'] = $row['tipoVehiculo'];
